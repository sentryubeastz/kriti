...
function updateTabUI() {
  ...
  if (notesView) {
    notesView.classList.toggle('active', activeTab === 'notes');
  }
  if (statsView) {
    statsView.classList.toggle('active', activeTab === 'stats');
  }
}
...
function createWordCard(pageTitle, note, query = '') {
  ...
  const translation = createMetaLine('Translation:', note.translation || 'Not available', query);
...
function renderStats() {
  const totalLookups = historyState.length;
  const totalSaved = getTotalSavedWords();

  setText('totalLookupsValue', String(totalLookups));
  setText('totalSavedValue', String(totalSaved));
  ...
  if (topLang) {
    const langCode = topLang[0];
    const label = LANGUAGE_LABELS[langCode] || String(langCode).toUpperCase();
...
function getTotalSavedWords() {
  return Object.keys(notesState).reduce((sum, title) => sum + (notesState[title] || []).length, 0);
}

function buildWordStats(history) {
  const map = new Map();
  history.forEach((entry) => {
    const normalized = String(entry.word || '').trim().toLowerCase();
    if (!normalized) {
      return;
    }
    if (!map.has(normalized)) {
      map.set(normalized, { word: String(entry.word).trim(), count: 0 });
    }
...
function buildLanguageStats(history) {
  const map = new Map();
  history.forEach((entry) => {
    const key = String(entry.language || '').trim().toLowerCase();
    if (!key) {
      return;
    }
...
function getWeeklyHistoryCounts(history) {
  ...
  history.forEach((entry) => {
    ...
    if (diff >= 0 && diff < 7 * dayMs) {
      thisWeek += 1;
    } else if (diff >= 7 * dayMs && diff < 14 * dayMs) {
      lastWeek += 1;
    }
  });
